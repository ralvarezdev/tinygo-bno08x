...
func NewBNO08X(
	...
	// Perform reset
	if err := bno08x.Reset(); err != tinygoerrors.ErrorCodeNil {
		return nil, ErrorCodeBNO08XFailedToResetBNO08X
	}
	return bno08x, tinygoerrors.ErrorCodeNil
}
...
func (b *BNO08X) Reset() tinygoerrors.ErrorCode {
	...
	for i := 0; i < ResetAttempts; i++ {
		...
		// Software reset
		if err := b.softwareReset(); err != tinygoerrors.ErrorCodeNil {
			return err
		}
		...
		if b.afterResetFn != nil {
			if err := b.afterResetFn(b); err != tinygoerrors.ErrorCodeNil {
				return err
			}
		}
...
func (b *BNO08X) checkID() tinygoerrors.ErrorCode {
...
func (b *BNO08X) waitForPacketType(
	...
	for time.Since(startTime) < WaitForPacketTypeTimeout {
		...
		if newPacket.ChannelNumber() == channelNumber {
			if reportID == nil {
				return newPacket, tinygoerrors.ErrorCodeNil
			}
...
func (b *BNO08X) handlePacket(packet Packet) tinygoerrors.ErrorCode {
...
func (b *BNO08X) processReport(report report) tinygoerrors.ErrorCode {
...
func (b *BNO08X) processControlReport(report report) tinygoerrors.ErrorCode {
...
func (b *BNO08X) EnableFeature(featureID uint8) tinygoerrors.ErrorCode {
	for i := 0; i < EnableFeatureAttempts; i++ {
		...
		if ok && !b.IsFeatureEnabled(featureDependency) {
			if err := b.EnableFeature(featureDependency); err != tinygoerrors.ErrorCodeNil {
				continue
			}
		}
...
func (b *BNO08X) BeginCalibration() tinygoerrors.ErrorCode {
	...
	if err := b.sendMeCommand(commandParametersBuffer); err != tinygoerrors.ErrorCodeNil {
...
func (b *BNO08X) sendMeCommand(subcommandParams []byte) tinygoerrors.ErrorCode {
...
func (b *BNO08X) SaveCalibrationData() tinygoerrors.ErrorCode {
